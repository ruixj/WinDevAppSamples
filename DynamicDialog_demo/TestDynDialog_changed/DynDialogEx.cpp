...
BOOL CDynDialogEx::OnCommand(WPARAM wParam, LPARAM lParam) 
{
	...
	CRect   rcTemp;
	CRect   Rect(FIXEDCOL1, m_nCurRow, FIXEDCOL2, m_nCurRow + ROWSTEPSIZE),rect2;
	//m_nCurRow += ROWSTEPSIZE;
	//m_nCurRow += 5;
	// CRect rect2(Rect);
	 //MapDialogRect(&rect2);
	...
	if (wControlID != 0) {
		...
		case IDC_DYN_BUTTON:
           ...
           // rcTemp.TopLeft()           =    CPoint(m_DialogTemplate.x,   m_DialogTemplate.y);
			//rcTemp.BottomRight()   =   CPoint(m_DialogTemplate.cx,   m_DialogTemplate.cy+ROWSTEPSIZE);
			  
            GetWindowRect(&rcTemp);
			rcTemp.bottom = rcTemp.bottom + ROWSTEPSIZE+ROWSTEPSIZE + 5;
			...
			if(myStatic  == NULL)
			{
			    Rect.left = FIXEDCOL1;
				Rect.top = m_nCurRow;
				Rect.right = FIXEDCOL2;
				Rect.bottom = m_nCurRow + ROWSTEPSIZE;
				m_nCurRow += ROWSTEPSIZE;
				m_nCurRow += 5;
				myStatic = new CStatic;
				myStatic->Create(_T("my static"), WS_CHILD|WS_VISIBLE|SS_CENTER,Rect, this);
				 rect2 = Rect;
				 MapDialogRect(&rect2);
				 myStatic->SetWindowPos(NULL, rect2.left, rect2.top, rect2.Width(), rect2.Height(), SWP_NOZORDER);
			}
			MoveWindow(&rcTemp);  
